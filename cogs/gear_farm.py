...
PROMPT = """На картинке ДОЛЖЕН быть скриншот от Discord-бота C3PO (команда inventory unit,
Star Wars: Galaxy of Heroes) — список недостающих деталей снаряжения/релик-материалов
персонажа. Характерный вид: заголовки "Additional Gear needed for G<N>" (детали снаряжения)
и/или "Additional Material needed for R<N>" (релик-материалы, числа N любые — это целевой
гир/уровень реликвии), под каждым — строки с иконкой детали слева, названием и количеством
через "x" справа.

СНАЧАЛА проверь: это действительно такой скриншот, или что-то другое (случайная картинка,
другой скриншот игры/чата, мем, пустое/нечитаемое изображение и т.п.)?

Если это НЕ такой скриншот — верни ровно:
{"is_c3po_inventory": false, "gear_tier": null, "relic_tier": null, "items": []}
и дальше ничего не выдумывай.

Если это такой скриншот — распознай ВСЕ строки из ОБОИХ разделов (какие есть; один из
разделов может отсутствовать, если по нему всё уже готово) — название детали на английском,
как написано на картинке, и количество после "x".

Верни СТРОГО JSON без markdown-обрамления (без ```), ровно такой структуры:
{
  "is_c3po_inventory": true,
...
class GearFarm(commands.Cog):
    ...
    async def gear_farm(
        ...
        # Явная проверка "это вообще нужный скриншот" (модель сама решает по виду картинки,
        # см. PROMPT) — до неё картинка, никак не похожая на C3PO (мем, случайный скрин),
        # могла пройти дальше как "0 деталей" или, хуже, модель начинала галлюцинировать
        # правдоподобно выглядящие названия. is_c3po_inventory по умолчанию False (строго),
        # если модель вдруг не вернула поле, а не True — не хотим молча доверять непонятно чему.
        if not data.get("is_c3po_inventory", False):
            await inter.edit_original_response(
                "❌ На картинке не похоже на список деталей от C3PO (`inventory unit`) — "
                "проверьте ссылку на сообщение."
            )
            return
        ...
        if not items:
            await inter.edit_original_response(
                "⚠️ Похоже на скриншот C3PO, но не удалось распознать ни одной детали — "
                "попробуйте другую ссылку или проверьте картинку вручную."
            )
            return
