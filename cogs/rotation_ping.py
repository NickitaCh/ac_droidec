...
class RotationPing(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # Защита от дублирования: для каждого текста храним последнюю минуту отправки
    ...
    async def rotation_ping_loop(self):
        ...
        for entry in self.parsed_schedule:
            ...
            # Проверяем, не отправляли ли уже это сообщение в эту минуту
            text = entry["text"]
            current_minute_key = f"{now_msk.strftime('%Y%m%d%H%M')}_{text}"
            if current_minute_key == self.last_sent_minutes.get(text):
            ...
            try:
                ...
                print(f"✅ [RotationPing] Тег '{text}' отправлен в {now_msk.strftime('%Y-%m-%d %H:%M')} МСК")
                # Запоминаем минуту, чтобы не повторить
                self.last_sent_minutes[text] = current_minute_key
