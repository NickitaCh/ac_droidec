...
# _parse_tb_plan_message). Не все гильдии, которых обслуживает бот, пишут ордер по
# нашему шаблону — этот паттерн ловит "1 этап"/"этап 1"/"1-й этап"/"Этап №1" и т.п.
# в любом порядке слов и с любым разделителем.
TB_ORDER_STAGE_RE = re.compile(
    r"(?P<num_before>\d+)[\s\-–—:]*(?:й[\s\-–—:]*)?этап\w*"
    r"|этап\w*[\s\-–—:№]*(?P<num_after>\d+)",
    re.IGNORECASE,
)
...
TB_PLANET_CONFLICT = {
    ...
    "датомир": "01",
    "медстанция": "01",
    "малахор": "01",  # опечатка, оставлена как алиас — правильно "малакор", см. ниже
    "малакор": "01",
    "звезда смерти": "01",
    "кореллия": "02",
    "фелуция": "02",
    ...
    "корусант": "03",
    "бракка": "03",
    "кашик": "03",  # опечатка, оставлена как алиас — правильно "кашиик", см. ниже
    "кашиик": "03",
    "лотал": "03",
    "кольцо кафрены": "03",
    "кольцо каферны": "03",
...
class GuildEvents(commands.Cog):
    ...
    # Источник ордера — одна ветка с планом на все 6 этапов разом (не по дням), в которой
    # бот ищет сообщение с номером нужного этапа (см. TB_ORDER_STAGE_RE — общий паттерн
    # "N этап"/"этап N", не завязанный на наш конкретный текст заголовка, т.к. у других
    # гильдий на этом же боте свой шаблон ордера) и публикует его целиком. Какая это
    # ветка: если офицер выбрал сохранённый план командой
