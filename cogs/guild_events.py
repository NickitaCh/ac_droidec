...
# Mixed, см. TB_ZONE_RE/TB_CONFLICT_LABELS). Офицеры перед каждой фазой вручную постят в
# #ac-тб-оповещения сообщение вида "Восход Империи — N этап" со списком реальных планет,
# каждая — под цветным маркером. Ветку определяем по САМОМУ маркеру (а не по порядку
# перечисления — офицер не обязан всегда писать в одном и том же порядке): 🔴=Light,
# 🟡/🟠=Dark, 🔵=Mixed, любой другой маркер (🟢/⚪/🟣/⚫/🟤) — бонусная/ОЗ-зона.
# Соответствие цвет->ветка проверено на сырых ключах comlink (ОЗ-миссии по трём фазам
# подряд стабильно попадают на conflict03/🔵) и на карте swgohrote.com (Малахор и Датомир,
# оба под 🔴 в разных фазах, лежат на одной и той же "красной" ветке карты).
# Бот слушает это сообщение и сохраняет соответствие (фаза, ветка) -> планета, командой
...
TB_PLAN_EMOJI_CONFLICT = {"🔴": "01", "🟡": "02", "🟠": "02", "🔵": "03"}
...
class GuildEvents(commands.Cog):
    ...
    def _parse_tb_plan_message(self, content: str):
        ...
        Ветка определяется по цвету маркера самой строки (см. TB_PLAN_EMOJI_CONFLICT),
        а не по порядку строк в сообщении — офицер не обязан каждый раз перечислять
        планеты в одном и том же порядке."""
        ...
        for line in content.splitlines():
            ...
            if not name:
                continue
            conflict_key = TB_PLAN_EMOJI_CONFLICT.get(stripped[0], "bonus")
