...
GUILD_SETTINGS_GROUPS = [
    {
        ...
        "hint": "Раз в две недели (когда наступает «тегаемая» неделя цикла ТБ) бот в заданные дни/время "
                "тегает роль в канале ротации напоминаниями «взводы» и «ордер» — чтобы никто не забыл выставить "
                "взвод и заказ перед стартом Территориальной битвы (само расписание и дата старта цикла пока не "
                "выведены сюда — задаются в коде (main.py) и общие для всех гильдий). Плюс канал анонсов плана ТБ "
                "(туда же падают автоордера), канал/ветка, откуда бот берёт стратегию по этапам, роль в "
                "автоордере, и канал, куда бот публикует автоотчёт по итогам ТБ.",
        "fields": [
            ("ping_channel_id", "Канал для тега на ротацию/взводы перед ТБ", "channel"),
            ("ping_role_id", "Роль, тегаемая на ротацию/взводы перед ТБ", "role"),
            ...
            ("officer_channel_id", "Канал для автоотчёта и уведомлений по ТБ", "channel"),
        ],
    },
    {
        "name": "День рождения",
