...
def slot_candidates(
    *, owners: list, base_id: str, here: tuple, used_pairs: dict,
    ...
    max_per_planet_per_round: int = MAX_UNITS_PER_PLANET_PER_ROUND,
) -> list:
    """owners — [{"ally_code","name","relic"}, ...] (уже топ-N по релику — см.
    ...
    filter_rules — tb_platoon_filters.ParsedRules или None (без фильтров).

    Возвращает owners, дополненные:
      meets_min — релик проходит порог этапа
...
def pick_best_candidate(candidates: list, bundle_preferred_codes: frozenset = frozenset()):
    ...
    релик > меньше всего уже занятых слотов на этой планете в этом этапе (размазывает
    нагрузку вместо того, чтобы наваливать на одного и того же топ-донора). Возвращает
