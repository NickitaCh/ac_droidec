...
def access_status_message(user: dict) -> str:
    ...
    if database.discord_id_has_any_registration(user["discord_id"]):
        return (
            ...
            'Подключить/продлить может офицер гильдии на странице <a href="/subscribe">Подписка</a>.'
        )
    return (
        ...
        'Если бот ещё не подключён к вашему серверу — офицер может подключить его и оплатить доступ на странице <a href="/subscribe">Подписка</a>.'
    )
