...
# Дублирует main.py::SITE_URL — тот же паттерн, что и N_LIMIT выше (веб-процесс
# не импортирует main.py, см. CLAUDE.md про дублирование operator-констант).
SITE_URL = "https://swgoh-sng.ru"


def access_status_message(user: dict) -> str:
    """Онбординг вместо голого "доступа нет" на / для tier != officer — веб-аналог
    main.py::_access_denied_message. Отличие от Discord-версии: там есть
    inter.guild_id (конкретный сервер, где вызвана команда), тут после OAuth
    известен только discord_id без привязки к серверу — поэтому вместо
    get_guild_config_by_discord_id используется discord_id_has_any_registration,
    чтобы отличить "никогда не регистрировался" от "регистрировался, но его
    гильдия сейчас не активна" (resolve_access видит только активные гильдии,
    так что оба случая иначе неотличимы друг от друга).

    Возвращает готовый HTML (безопасно — весь текст свой, не пользовательский
    ввод) — рендерить в шаблоне через `{{ ... |safe }}`."""
    tier = user.get("tier")
    if tier == "member":
        return (
    ...
    if database.discord_id_has_any_registration(user["discord_id"]):
        return (
            "Ваша гильдия сейчас не подключена к боту либо подписка не активна.<br>"
            f'Подключить/продлить может офицер гильдии — тарифы и подключение бота на <a href="{SITE_URL}">главной странице</a>.'
        )
    return (
        "Вы ещё не зарегистрированы. Регистрация — только через Discord-бота: "
        "выполните <code>/регистрация &lt;код союзника&gt;</code> на сервере вашей гильдии.<br>"
        f'Если бот ещё не подключён к вашему серверу — офицер может подключить его и оплатить доступ на <a href="{SITE_URL}">главной странице</a>.'
    )
