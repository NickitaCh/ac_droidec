...
def _access_denied_message(inter: disnake.ApplicationCommandInteraction, tier: str | None) -> str:
    ...
    if inter.guild_id is None:
        return (
            ...
            f"Если гильдия ещё не подключена к боту — подключить и оплатить может офицер на странице {SITE_URL}/subscribe"
        )
    ...
    if guild_cfg is None:
        return (
            ...
            f"Подключить и оплатить доступ может офицер — на странице {SITE_URL}/subscribe или командой `/гильдия_заявка`."
        )
    if not guild_cfg["is_active"]:
        return (
            "🛑 Подписка этой гильдии на бота сейчас не активна (не оплачена или истекла).\n"
            f"Продлить может офицер — на странице {SITE_URL}/subscribe."
        )
